...
use cosmic::{
    Element,
    applet,
    iced::{Alignment, Length, widget::{column, row}},
...
impl PanelFormatter {
    ...
    pub fn vertical_layout<'a, M: 'a>(
        ...
        let elements: Vec<Element<'a, M>> = if let Some(strftime) = 
            self.maybe_strftime(now, &config.format_strftime) 
        {
            strftime
                .split_whitespace()
        ...
        Element::from(
            column!(
                ...
                horizontal_space().width(Length::Fixed(
                    (applet.suggested_size(true).0
                        + 2 * applet.suggested_padding(true).1)
                        as f32
                ))
            )
            .align_x(Alignment::Center),
    ...
    pub fn horizontal_layout<'a, M: 'a>(
        ...
        let formatted_date = if let Some(strftime) = 
            self.maybe_strftime(now, &config.format_strftime) 
        {
            strftime
        } else {
